...
// Multer Setup (Handles both Thumb & Download File in one request)
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        cb(null, file.fieldname === 'downloadFile' ? downloadDir : uploadDir);
    },
...
const upload = multer({
    ...
    fileFilter: (req, file, cb) => cb(null, true) // Allow all for downloads, restrict thumb if needed
});

// DB Schema & Migration
db.exec(`
    CREATE TABLE IF NOT EXISTS tools (id INTEGER PRIMARY KEY, name TEXT, desc TEXT, thumb TEXT, status TEXT, visible INTEGER, download_type TEXT, download_value TEXT, downloads INTEGER);
...
if (!db.prepare('SELECT id FROM admin').get()) {
    ...
    db.prepare('INSERT OR IGNORE INTO settings (key, value) VALUES (?, ?)').run('footer_text', 'Secure Tools. Direct Access. No Compromises.');    db.prepare('INSERT OR IGNORE INTO settings (key, value) VALUES (?, ?)').run('logo', '');
}

app.set('view engine', 'ejs');
app.use(express.static(path.join(__dirname, 'public')));
app.use('/downloads', express.static(downloadDir)); // Serve download files
...
app.get('/admin', requireAuth, (req, res) => {
    const s = getSettings();
    res.render('admin', { tools: db.prepare('SELECT * FROM tools ORDER BY id DESC').all(), socials: db.prepare('SELECT * FROM socials').all(), settings: s, site_title: s.site_title });
});
...
app.post('/api/admin/tools', requireAuth, upload.fields([{ name: 'thumbFile', maxCount: 1 }, { name: 'downloadFile', maxCount: 1 }]), (req, res) => {
    try {
        ...
        let dlType = 'url', dlValue = downloadUrl || '#';

        if (req.files?.thumbFile?.[0]) thumb = `/uploads/${req.files.thumbFile[0].filename}`;
        if (req.files?.downloadFile?.[0]) {
            dlType = 'file';            dlValue = `/downloads/${req.files.downloadFile[0].filename}`;
        }
...
app.post('/api/admin/tools/update', requireAuth, upload.fields([{ name: 'thumbFile', maxCount: 1 }, { name: 'downloadFile', maxCount: 1 }]), (req, res) => {
    try {
        ...
        let dlType = req.body.downloadType || 'url', dlValue = downloadUrl || currentDownload || '#';

        if (req.files?.thumbFile?.[0]) thumb = `/uploads/${req.files.thumbFile[0].filename}`;
        if (req.files?.downloadFile?.[0]) {
            dlType = 'file';
            dlValue = `/downloads/${req.files.downloadFile[0].filename}`;
        }
...
// Settings & Socials (Form-based, no refresh needed but kept simple)
